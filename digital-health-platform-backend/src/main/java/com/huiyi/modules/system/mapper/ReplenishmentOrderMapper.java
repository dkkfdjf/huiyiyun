...
public interface ReplenishmentOrderMapper extends BaseMapper<ReplenishmentOrder> {
    ...
    /**
     * 台账入库汇总:按 药品×网点 分组 SUM 入库量。可选公司/药品/网点/时间窗口过滤。
     * replenishment_order 为 append-only(无 deleted 列),无需软删过滤。
     */
    @Select("<script>" +
            ...
            "  <if test='start != null'>AND in_time &gt;= #{start}</if>" +
            "  <if test='end != null'>AND in_time &lt;= #{end}</if>" +
            "</where>" +
